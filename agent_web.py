...
with st.sidebar:
    ...
    # To jest linia, która wcześniej powodowała błąd - teraz jest pełna
...
# 1. Liderzy (Top 5 indeksu)
st.write("**Top 5 Spółek (Indeks):**")
cols = st.columns(5)
...
# 2. Największe Wzrosty (HORYZONTALNIE)
st.markdown("### 🟩 Top 5 Wzrostów (Miesiąc)")
if gainers:
    g_cols = st.columns(5) # 5 kolumn obok siebie
...
# 3. Największe Spadki (HORYZONTALNIE)
st.markdown("### 🔻 Top 5 Spadków (Miesiąc)")
if losers:
    l_cols = st.columns(5) # 5 kolumn obok siebie
    for i, l in enumerate(losers):
        with l_cols[i]:
            st.metric(
                ...
                f"{l['price']:.2f}", 
                f"{l['month_change']:.2f}%",
                delta_color="normal" # Czerwony (bo wartość jest ujemna)
            )
else:
    st.write("Brak danych.")
