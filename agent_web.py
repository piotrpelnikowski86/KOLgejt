...
BACKUP_SP500_LARGE = ["AAPL", "MSFT", "NVDA", "GOOGL", "AMZN", "META", "TSLA", "BRK-B", "LLY", "AVGO", "JPM", "V", "XOM", "UNH", "MA", "PG", "JNJ", "HD", "MRK", "COST", "ABBV", "CVX", "CRM", "BAC", "WMT", "AMD", "ACN", "PEP", "KO", "LIN", "TMO", "DIS", "MCD", "CSCO", "ABT", "INTC", "WFC", "VZ", "NFLX", "QCOM", "INTU", "NKE", "IBM", "PM", "GE", "AMAT", "TXN", "NOW", "SPGI", "CAT"]
...
# --- POBIERANIE LIST (CACHE V10) ---
@st.cache_data(ttl=3600)
def get_full_market_tickers_v10(market):
    headers = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64)"}
    if market == "WIG20": return POOL_WIG20
    ...
    if market == "S&P 500":
        ...
        except: return BACKUP_SP500_LARGE
    return []

# --- FUNDAMENTY (CACHE V10) ---
@st.cache_data(ttl=3600*4)
def scan_fundamentals_v10(tickers_list):
    fundamental_data = []
    strong_buys = []
...
def get_market_overview_v10(tickers):
    try:
        ...
        for t in preview:
            try:
                if len(data[t]) > 10:
                    c, s = data[t]['Close'].iloc[-1], data[t]['Close'].iloc[0]
                    changes.append({"t": t, "mc": ((c-s)/s)*100, "p": c})
            except: pass
...
if "WIG20" in market_choice: market="WIG20"; tickers_scan=get_full_market_tickers_v10("WIG20"); tickers_fund=POOL_WIG20
elif "Nasdaq" in market_choice: market="Nasdaq 100"; tickers_scan=get_full_market_tickers_v10("Nasdaq 100"); tickers_fund=POOL_NASDAQ
else: market="S&P 500"; tickers_scan=get_full_market_tickers_v10("S&P 500"); tickers_fund=POOL_SP500

st.subheader(f"🔥 Przepływ Rynku: {market}")
with st.spinner("Analiza trendów..."): gainers, losers = get_market_overview_v10(tickers_scan)

st.write("**🚀 Top Wzrosty (Miesiąc)**")
if gainers:
    html = '<div class="scroll-container">'
    for g in gainers:
        # SPŁASZCZONY HTML - TU BYŁ BŁĄD WCZEŚNIEJ
...
with st.spinner("Szukam perełek fundamentalnych (Smart Pool)..."):
    top_funds, best_pick = scan_fundamentals_v10(tickers_fund)
